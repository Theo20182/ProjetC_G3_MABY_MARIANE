...
void defineState(Graph* G, int id, int state) {
    Node* temp = G->list_successors[id];
    while (G->list_successors[id] != NULL) {
        G->list_successors[i]->state == state;
        G->list_successors[id] = G->list_successors[id]->successor;
    }
    G->list_successors[id] = temp;
}
...
void immune(Graph* G, int id){
    printf("id: %d\n", id);
    if((G->list_successors[i]->state == 1) && (G->list_successors[id]->force >= 40)){
        Node* temp = G->list_successors[id];
        while (G->list_successors[id] != NULL) {
            G->list_successors[i]->state == 2;
            G->list_successors[id] = G->list_successors[id]->successor;
        }
        G->list_successors[id] = temp;
    }
...
void defineVampire(Graph* G, int id, int time){
    printf("id: %d\n", id);
    if((G->list_successors[i]->state == 2) && (time >= 3)) {
        defineState(G, id, 4);
    }
}
...
void vampireAttack(Graph* G, id_vampire) {
    int victime = rand()%11;
    int tab[victime];
    ...
    for(int i = 0; i =< victime; i++) {
        id = tab[i];
        switch(G->liste_successors[id]->state) {
            case 0:
                defineState(G, id, 5);
            break; 
